...
* **Foundry VTT:** v14.364+ (проверено на v14.367)
* **Система Daggerheart для Foundry (Foundryborne):** 2.9.1+ (базовая проверка в игре на 2.9.3)
...
* Контент **Hope & Fear**, перенесённый из беты **The Void**, локализован прямо в официальных компедиумах Foundryborne и не требует отдельного контент-модуля. Старые beta-переводы сохранены только для контента, который не вошёл в систему.
...
Релиз `2026.09.08` — первый этап поддержки Hope & Fear, а не полный перевод дополнения. Перенесены доступные переводы опций персонажей и ранее переведённого контента The Void. Часть новых противников, окружений, снаряжения и отдельных свойств пока отображается по-английски.

Некоторые описания ранее переведённых противников и окружений временно адаптированы нами по актуальным английским правилам Foundryborne; это не перевод команды сайта. Опубликованные тексты сайта будут включаться в последующие обновления после сверки.

Перед обновлением системы и модуля сделайте резервную копию мира. Для перенесённого контента используйте официальные библиотеки `daggerheart`: старые библиотеки The Void/Voidborne могут содержать отдельные версии тех же карточек. Обновление перевода не заменяет автоматически ранее импортированные в мир документы; старые контент-модули не отключайте без проверки зависимостей вашего мира.
...
Исправленные локально описания противников и окружений Hope & Fear пока служат временным переводом. Они не закреплены как постоянные переопределения: когда соответствующие тексты появятся в API сайта, `npm run update:translations:full` заменит названия и описания, включая описания вложенных свойств. Пустые или отсутствующие тексты API сохраняют существующие описания. Для официальных паков свойства сопоставляются по английским названиям из `original` и ID особенностей API, а не по порядку в списке; если соответствие не найдено, перевод остаётся без изменений.

Консервативный `npm run update:translations` вложенные свойства не обновляет. Полный проход запускайте только явно, после обновления API-кэша, и проверяйте его diff: действия, эффекты, технические теги и ручные особенности требуют отдельного контроля. Появление перевода на сайте само по себе не обновляет установленный модуль. Полностью отсутствующие переводы нового контента пока остаются в ожидании команды сайта.
